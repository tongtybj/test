...
public:
  TensorRT(std::string encoder_model_file, std::string decoder_model_file, float score_threshold, float cosine_window_factor, int cosine_window_step, float size_lpf_factor);
  ~TensorRT(){}
  ...
  void init(const std::vector<float>& bbox, const cv::Mat& img);
  void track(const cv::Mat& img);

private:
  nvinfer1::ICudaEngine* loadEngine(const std::string& model_file, std::stringstream& err_msg);
  ...
  void processInput(cv::Mat& input_img, std::vector<float>& mask_bounds, std::string image_type);
